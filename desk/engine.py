        ...
        for t in self.store.open_trades():
            ...
            if ts.time() >= force_at:
                ...
                if crush and entry_g.get("iv"):
                    ...
                    if drop >= crush:
                        reason = "IV_CRUSH"
                # 9-EMA trailing exit (the doc's advanced exit): leave when
                # the underlying closes back through its 5m 9-EMA against us.
                if not reason and ex.get("ema9_trail"):
                    side_now = snapshots[t["symbol"]].get("_features", {}).get("ema9_side")
                    want = 1.0 if t["opt_type"] == "CE" else -1.0
                    if side_now is not None and side_now == -want:
                        reason = "EMA9_TRAIL"
