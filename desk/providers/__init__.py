...
def get_provider(cfg: dict):
    ...
    if want in ("kite", "auto"):
        try:
            ...
            prov = KiteProvider()
            # Fail loud and clear if the daily token is dead, rather than
            # letting a later empty chain look like an NSE block.
            try:
                prov.kite.profile()
            except Exception as exc:
                raise RuntimeError(
                    "KITE TOKEN LOOKS EXPIRED — refresh KITE_ACCESS_TOKEN. "
                    "The Kite access token dies ~07:30 IST daily. "
