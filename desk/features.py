...
FEATURE_DOC = {
    ...
    "opening_range_break": "-1 below OR low, 0 inside, +1 above OR high (by price)",
    "orb_body_break": ("-1/0/+1 but by the 5m candle BODY, not the wick. +1 only "
                       "when a completed body closes above the 15-min opening "
                       "range high. This is the doc's body-breakout filter - a "
                       "wick poking through does not count."),
    "orb_range_pct": "the opening range width as % of price (thin OR = weak signal)",
    "dist_from_or_pct": "% distance of price beyond the OR edge it broke",
    "rel_volume": ("today's opening-range volume vs the 14-session average for "
                   "this instrument. Above ~1.5 is the doc's 'Stock in Play'."),
    "ema9_side": "+1 price above the 5m 9-EMA, -1 below (the doc's trailing tool)",
...
def build(snapshot: dict, candles: list[dict], cost_model, cfg: dict,
    ...
    if today:
        ...
        f["orb_range_pct"] = round(100.0 * (or_hi - or_lo) / spot, 4) if spot else 0.0

        # Body-close breakout: the last CLOSED 5m candle's body vs the OR.
        last = today[-1]
        body_hi, body_lo = max(last["o"], last["c"]), min(last["o"], last["c"])
        f["orb_body_break"] = 1.0 if body_lo > or_hi else (
            -1.0 if body_hi < or_lo else 0.0)
        if f["orb_body_break"] > 0:
            f["dist_from_or_pct"] = round(100.0 * (spot - or_hi) / or_hi, 4)
        elif f["orb_body_break"] < 0:
            f["dist_from_or_pct"] = round(100.0 * (or_lo - spot) / or_lo, 4)
        else:
            f["dist_from_or_pct"] = 0.0

        # Relative volume: opening-range volume today vs its own recent baseline.
        or_vol = sum(c["v"] for c in orb)
        prior = {}
        for c in candles:
            d = c["t"][:10]
            hhmm = c["t"][11:16]
            if "09:15" <= hhmm <= "09:30":
                prior.setdefault(d, 0.0)
                prior[d] += c["v"]
        hist = [v for d, v in prior.items() if d != snapshot["ts"][:10] and v > 0]
        base = statistics.fmean(hist[-14:]) if hist else 0.0
        f["rel_volume"] = round(or_vol / base, 3) if base > 0 else 1.0

        ema9 = _ema([c["c"] for c in today], 9)
        f["ema9_side"] = 1.0 if spot >= ema9 else -1.0
    else:
        f["range_pos"], f["vwap_dev_pct"], f["opening_range_break"] = 0.5, 0.0, 0.0
        f["orb_body_break"] = 0.0
        f["orb_range_pct"] = 0.0
        f["dist_from_or_pct"] = 0.0
        f["rel_volume"] = 1.0
        f["ema9_side"] = 0.0
